...
// Aurora color palettes (RGB values) - dark mode (bright/high lightness)
interface RGB {
  r: number;
  g: number;
  b: number;
...
// Aurora color palettes - light mode (darker/lower lightness for visibility)
const AURORA_COLORS_LIGHT: RGB[][] = [
  // Classic green aurora (darkened)
  [
    { r: 0, g: 140, b: 75 },
    { r: 30, g: 140, b: 95 },
    { r: 0, g: 110, b: 55 },
  ],
  // Blue-purple aurora (darkened)
  [
    { r: 55, g: 80, b: 140 },
    { r: 85, g: 55, b: 140 },
    { r: 45, g: 65, b: 110 },
  ],
  // Pink-magenta aurora (darkened)
  [
    { r: 140, g: 55, b: 100 },
    { r: 140, g: 85, b: 110 },
    { r: 110, g: 45, b: 85 },
  ],
  // Mixed aurora (green-blue-purple, darkened)
  [
    { r: 0, g: 140, b: 75 },
    { r: 55, g: 110, b: 140 },
    { r: 100, g: 55, b: 140 },
...
interface AuroraState {
  ...
  backgroundAlpha: number; // Variable transparency for background
}
...
const actor: Actor = {
  ...
  async setup(api: ActorSetupAPI): Promise<void> {
    ...
    state.colorPaletteIndex = Math.floor(Math.random() * AURORA_COLORS_DARK.length);
    state.paletteTransitionProgress = 0;
    state.backgroundAlpha = 0.1 + Math.random() * 0.65; // 0.1 to 0.75

    console.log(`[aurora-dreamscape] Setup complete with ${state.curtains.filter(c => c.active).length} curtains, bgAlpha=${state.backgroundAlpha.toFixed(2)}`);
  },
  ...
  update(api: ActorUpdateAPI, frame: FrameContext): void {
    ...
    if (state.paletteTransitionProgress >= 1) {
      state.paletteTransitionProgress = 0;
      state.colorPaletteIndex = (state.colorPaletteIndex + 1) % AURORA_COLORS_DARK.length;
    }

    // Intensity pulsing
    state.intensity = 0.7 + Math.sin(time * 0.2) * 0.2 + Math.sin(time * 0.5) * 0.1;

    // ============ Draw sky background (mode-aware) ============

    const isDarkMode = api.context.display.isDarkMode();

    // Linear gradient for sky (top to bottom)
    // This is a background fill, so use normal blend mode
    api.brush.rect(0, 0, width, height, {
      fill: {
        ...
        stops: isDarkMode
          ...
          : [
              { offset: 0, color: '#e8e8f0' },
              { offset: 0.5, color: '#f0f0f8' },
              { offset: 1, color: '#f8f8fc' },
            ],
      },
      alpha: state.backgroundAlpha,
    });

    // ============ Draw stars using pre-rendered glow texture (mode-aware) ============

    // Mode-aware blend mode for additive effects
    const additiveBlend = isDarkMode ? 'add' : 'multiply';
    ...
    for (let i = 0; i < state.stars.length; i++) {
      ...
      if (star.size > 1.2) {
        ...
        api.brush.image(state.glowTexture, star.x, star.y, {
          width: glowSize,
          height: glowSize,
          // Dark mode: soft blue-white, Light mode: darker blue-gray
          tint: isDarkMode ? 0xc8dcff : 0x4050a0,
          alpha: currentBrightness * 0.5 * alphaMultiplier,
          blendMode: additiveBlend,
        });
      }

      // Draw star core (small bright circle)
      // Dark mode: white, Light mode: dark
      api.brush.circle(star.x, star.y, star.size * 0.6, {
        fill: isDarkMode ? 0xffffff : 0x303060,
        alpha: currentBrightness * alphaMultiplier,
      });
    }

    // ============ Draw aurora curtains (mode-aware) ============

    // Select appropriate color palette based on display mode
    const colorPalettes = isDarkMode ? AURORA_COLORS_DARK : AURORA_COLORS_LIGHT;
    const palette = colorPalettes[state.colorPaletteIndex];
    ...
    for (let i = 0; i < state.curtains.length; i++) {
      ...
      for (let r = 0; r < rayCount; r++) {
        ...
        const rayAlpha = curtain.alpha * state.intensity * 0.4 * alphaMultiplier;

        // Draw ray as solid color with alpha fade (mode-aware blend)
