...
// Background colors for analysis
const BG_COLOR_DARK = { r: 10, g: 10, b: 15 };    // Dark mode background (0x0a0a0f)
const BG_COLOR_LIGHT = { r: 245, g: 245, b: 250 }; // Light mode background (0xf5f5fa)
 ...
 * @param isDarkMode - Whether the display is in dark mode
...
function analyzeRegion(
  ...
  // Select background color based on display mode
  const BG_COLOR = isDarkMode ? BG_COLOR_DARK : BG_COLOR_LIGHT;
...
const actor: Actor = {
  ...
  update(api: ActorUpdateAPI, frame: FrameContext): void {
    ...
    if (state.snapshot && !state.snapshotPending) {
      ...
      // Determine display mode for background detection
      ...
      for (let i = 0; i < regionsToFill; i++) {
        ...
        if (job && region) {
          ...
          // Adjust lightness based on display mode and canvas brightness
          // Dark mode: high lightness (60-90%) for visibility on dark background
          // Light mode: low lightness (10-40%) for visibility on light background
          let brightnessAdjust = state.analysis.averageBrightness > 0.5 ? -10 : 10;

          const c1 = palette[colorIdx1];
          const c2 = palette[colorIdx2];

          let baseL: number;
          let complementL: number;

          if (isDarkMode) {
            // Dark mode: shift colors to be lighter (60-85%)
            baseL = Math.max(60, Math.min(85, c1.l + brightnessAdjust + 15));
            complementL = Math.max(60, Math.min(85, c2.l + brightnessAdjust + 15));
          } else {
            // Light mode: shift colors to be darker (15-40%)
            baseL = Math.max(15, Math.min(40, c1.l + brightnessAdjust - 25));
            complementL = Math.max(15, Math.min(40, c2.l + brightnessAdjust - 25));
          }

          const baseColor = hslToNumeric(c1.h, c1.s, baseL);
          const complementColor = hslToNumeric(c2.h, c2.s, complementL);
    ...
    for (let i = 0; i < state.paintJobs.length; i++) {
      ...
      for (let j = 0; j < job.elements.length; j++) {
        ...
        // Get display mode for blend mode selection
        const isDark = api.context.display.isDarkMode();

        // Select blend modes based on display mode:
        // Dark mode: 'screen' and 'add' brighten content against dark background
        // Light mode: 'multiply' and 'darken' darken content against light background
        const blendScreen = isDark ? 'screen' : 'multiply';
        const blendAdd = isDark ? 'add' : 'darken';

        // Draw element based on type and job style
        // Light mode needs slightly lower alpha since multiply/darken can be harsh
        const baseAlpha = element.alpha * 0.7;
        const alpha = isDark ? baseAlpha : baseAlpha * 0.8;
    ...
    // Dark mode: black shadow for depth
    // Light mode: lighter, more subtle shadow
    if (activeElementCount >= 1) {
      const displayIsDark = api.context.display.isDarkMode();
      const shadowIntensity = Math.min(0.25, 0.12 + (activeElementCount / 20) * 0.13);
      // Light mode uses lower intensity since shadows are more prominent on light backgrounds
      const adjustedIntensity = displayIsDark ? shadowIntensity : shadowIntensity * 0.6;
      api.filter.dropShadow(`rgba(0, 0, 0, ${adjustedIntensity})`, 5, 2, 2);
    }
  },
