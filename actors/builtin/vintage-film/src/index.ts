...
const actor: Actor = {
  ...
  update(api: ActorUpdateAPI, frame: FrameContext): void {
    ...
    // Scratches are light on dark mode (like real film scratches), dark on light mode
    const isDarkMode = api.context.display.isDarkMode();
    const scratchColor = isDarkMode ? 0xffffff : 0x000000;
    ...
    // Dust appears as dark specks on light backgrounds, light specks on dark backgrounds
    const dustColor = isDarkMode ? 0xd0c8c0 : 0x28231e;
    const dustAlphaMultiplier = isDarkMode ? 1.0 : 0.8;
    ...
    for (let i = 0; i < state.dustParticles.length; i++) {
      ...
      api.brush.circle(particle.x, particle.y, particle.size, {
        fill: dustColor,
        alpha: particle.alpha * dustAlphaMultiplier,
      });
    }

    // ============ Draw burn patches ============

    // Film burns: bright overexposure on dark mode, dark underexposure on light mode
    const burnBlendMode = isDarkMode ? 'add' : 'multiply';
    const burnOuterColor = isDarkMode ? 0xffc864 : 0x805020; // Yellow-orange / dark amber
    const burnCoreColor = isDarkMode ? 0xfffff0 : 0x604010; // Bright white / dark brown
    const burnAlphaMultiplier = isDarkMode ? 1.0 : 0.7;
    ...
    for (let i = 0; i < state.burnPatches.length; i++) {
      ...
      const alpha = patch.intensity * 0.6 * burnAlphaMultiplier;

      // Outer glow (yellowish in dark mode, dark amber in light mode)
      api.brush.circle(patch.x, patch.y, patch.radius * 1.2, {
        fill: burnOuterColor,
        alpha: alpha * 0.3,
        blendMode: burnBlendMode,
      });

      // Core (bright in dark mode, dark in light mode)
    ...
    // Sprocket holes: dark on both modes (they're physical holes in the film)
    // But we adjust alpha for visibility
    const sprocketColor = isDarkMode ? 0x000000 : 0x1a1a1a;
    const sprocketAlpha = isDarkMode ? 0.8 : 0.6;
    ...
    // ============ Draw film edge darkening/lightening ============

    // Edge effect: darkening on dark mode, subtle lightening on light mode
    const edgeColor = isDarkMode ? 0x000000 : 0xffffff;
    const edgeAlphaBase = isDarkMode ? 0.15 : 0.1;
    const edgeAlphaStep = isDarkMode ? 0.04 : 0.03;

    // Top edge gradient effect
    for (let i = 0; i < 3; i++) {
      const edgeAlpha = edgeAlphaBase - i * edgeAlphaStep;
      const y = i * 8;
